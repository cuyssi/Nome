...
export const TomorrowSubjects = ({ bag, isOpen, onClose, onUpdateBag }) => {
    ...
    return (
        <div className="relative w-full bg-black border border-purple-600 rounded-xl p-6 max-w-md text-white shadow-lg">
            ...
            {subjects.length === 0 ? (
                <p className="font-poppins text-red-400 text-center text-sm mt-6">No hay asignaturas para mañana</p>
            ) : (
                <ul className="space-y-5 mt-10 ml-5">
                    {subjects.map((subject, i) => (
                        <li key={i} className="flex items-center gap-3">
                            <input
                                type="checkbox"
                                checked={!!bag.items?.[dayKey]?.includes(subject.nombre)}
                                onChange={() => toggleSubject(subject.nombre)}
                                className="accent-purple-400 w-5 h-5"
                            />
                            <span
                                className={`text-lg ${
                                    bag.items?.[dayKey]?.includes(subject.nombre) ? "line-through" : ""
                                }`}
                            >
                                {subject.nombre}
                            </span>
