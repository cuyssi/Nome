        ...
        <thead>
          <tr>
            ...
            {(days || []).map((day) => (
              <th
                key={day}
                className="bg-black text-purple-600 font-bold p-2 text-center"
              >
                {day}
              </th>
            ))}
        ...
        <div className="mt-14 flex gap-4 ml-4">
          ...
          <button
            onClick={() => {
              ...
              const hourToRemove = prompt(
                `Enter the hour to remove:\n${hours.join(", ")}`
              );
              if (hourToRemove && hours.includes(hourToRemove))
