...
export const Task_card = ({ task, onDelete, onEdit }) => {
    const { dragOffset, handleTouchStart, handleTouchMove, handleTouchEnd, isRemoving, isEdited, color } = useTaskCard(
        task,
        onDelete,
        onEdit
    );
    ...
    return (
        <div className="relative w-full min-h-[6rem] overflow-hidden rounded-xl">
            <div className={`absolute inset-0 w-full z-0 flex items-center justify-between rounded-xl transition-colors duration-150 ease-in ${
  dragOffset > 120 ? "bg-red-900"
  : dragOffset > 80 ? "bg-red-500"
  : dragOffset > 0 ? "bg-red-400"
  : dragOffset < -120 ? "bg-yellow-900"
  : dragOffset < -80 ? "bg-yellow-500"
  : dragOffset < 0 ? "bg-yellow-200"
  : "bg-gray-400"
}`}>
