...
export const TaskSearch = ({ openModalWithTask }) => {
  ...
  const filteredTasks = tasks.filter((task) =>
    normalizeText(task.text).includes(normalizeText(query))
  );
      ...
      {filteredTasks.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {filteredTasks.map((task) => (
            <li
              key={task.id}
              className="border-b py-1 flex justify-between items-center text-gray-300"
            >
              <div className="flex gap-1 items-center">
                ...
                <span className={task.completed ? "line-through text-green-400" : ""}>
                  {task.text}
                </span>
              </div>
